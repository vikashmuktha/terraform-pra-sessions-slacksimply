...
output "instance_publicip" {
  ...
  #value = aws_instance.myaws.*.public_ip   # Legacy Splat
  #value = aws_instance.myaws[*].public_ip  # Latest Splat
  value = toset([for instance in aws_instance.myaws: instance.public_ip])
}

# EC2 Instance Public DNS with TOSET
output "instance_publicdns" {
  description = "EC2 Instance Public DNS"
  #value = aws_instance.myaws[*].public_dns  # Legacy Splat
  #value = aws_instance.myaws[*].public_dns  # Latest Splat
  value = toset([for instance in aws_instance.myaws: instance.public_dns])
}


# EC2 Instance Public DNS with TOMAP
output "instance_publicdns2" {
  value = tomap({for az, instance in aws_instance.myaws: az => instance.public_dns})
}
